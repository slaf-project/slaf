...
@pytest.fixture
def tiny_adata():
    """Load the permanent tiny sample AnnData object from disk"""
    h5ad_path = Path("../slaf-datasets/tiny_sample_dataset.h5ad")
    if not h5ad_path.exists():
        pytest.skip(
            "Tiny sample dataset h5ad not found. Run create_tiny_sample_dataset.py first."
        )
...
@pytest.fixture
def tiny_slaf():
    """Load the permanent tiny sample SLAF dataset as a SLAFArray"""
    slaf_path = Path("../slaf-datasets/tiny_sample_dataset.slaf")
    if not slaf_path.exists():
        pytest.skip(
            "Tiny sample dataset slaf not found. Run create_tiny_sample_dataset.py first."
        )
...
@pytest.fixture
def tiny_slaf_path():
    """Get the path to the permanent tiny sample SLAF dataset"""
    slaf_path = Path("../slaf-datasets/tiny_sample_dataset.slaf")
    if not slaf_path.exists():
        pytest.skip(
            "Tiny sample dataset slaf not found. Run create_tiny_sample_dataset.py first."
